#!/bin/bash
cd "$(dirname $0)/.." 
Gmethods=(iter_norm_wm cholesky_wm)
Gm=(0)
LRs=(2e-4 1e-4 1e-3)
BatchSize=(64)
seeds=(1)
   ...
   do 
      ...
      do	
        ...
        do
          for ((b=0;b<$f;++b))
          do
                name="exp2_1_${dataset}_${arc}_E_sn${discriminator_spectral}_G${Gmethods[$a]}_${generator_block_coloring}_Gins${g_instance_norm}_GW${Gm[$j]}_f${discriminator_filters}_bs${BatchSize[$k]}_${LRs[$i]}_s${seeds[$b]}_C${Count}"
   	            echo "${name}"
